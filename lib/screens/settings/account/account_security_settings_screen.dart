...
class _AccountSecuritySettingsScreenState extends State<AccountSecuritySettingsScreen> {
  ...
  void initState() {
    ...
    _showSecurityNotifications = _prefs.then((SharedPreferences prefs) {
      return (prefs.getBool(SharedPreferencesHelpers.showSecurityNotifications) ??
  ...
  Widget build(BuildContext context) {
    const String _firstText = 'Your messages and calls are secured with '
        'end-to-end encryption, which means '
        'WhatzApp and third parties can\'t read or '
        'listen to them. ';
    const String _lastText = 'Turn on this setting to receive '
  ...
  _setShowSecurityNotifications(bool value) async {
    ...
    setState(() {
      _showSecurityNotifications =
          prefs.setBool(SharedPreferencesHelpers.showSecurityNotifications, value).then((bool success) {
        return value;
      });
    });
  }
